...
export default class PositionComponent extends SimpleTransformComponent {
    ...
    @property({ type: Enum(PositionType) })
    ...
    protected applyTransform(
        ...
        if (this.positionType === PositionType.Relative) {
            ...
            newPosition = this.calculatePositionByRefSize(
                targetPosition,
                referenceSize
            );
            ...
            newPosition = targetPosition
                .clone()
                .multiplyScalar(
                    this.layer === LayerType.UI ? this.settings.SCALE : 1
                );
        }
    ...
    private applyOffset(targetPosition: Vec2, offset: Vec2, layer: LayerType) {
        return targetPosition.add(
            offset.multiplyScalar(
                layer === LayerType.UI ? this.settings.SCALE : 1
            )
        );
    }
    ...
    private calculatePositionByRefSize(
        relativePosition: Vec2,
        referenceSize: Size
    ) {
        return math.v2(
            referenceSize.width * relativePosition.x,
            referenceSize.height * relativePosition.y
        );
    }

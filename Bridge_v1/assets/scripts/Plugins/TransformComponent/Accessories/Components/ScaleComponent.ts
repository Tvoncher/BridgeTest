...
export default class ScaleComponent extends SimpleTransformComponent {
    ...
    @property({ type: Enum(SizeMode) }) private _sizeMode: SizeMode =
        SizeMode.Relative;
    @property({ type: Enum(FitMode) }) private _fitMode: FitMode =
        FitMode.Inscribe;
    @property({ type: Enum(LayerType) }) private _layer: LayerType =
        LayerType.UI;
    ...
    public get fitMode(): FitMode {
        return this._fitMode;
    }
    public set fitMode(v: FitMode) {
        this._fitMode = v;
    }
    ...
    public get desiredSize(): Vec2 {
        return this._desiredSize;
    }
    public set desiredSize(v: Vec2) {
        this._desiredSize = v;
    }
    ...
    @property({
        type: Vec2,
        visible() {
            return this.sizeMode === SizeMode.Relative && this.isActive;
        },
    })
    ...
    protected applyTransform(
        ...
        let referenceSize: Vec2 =
            this.checkTransformReference(transformComponent);
        ...
        newScale = this.calculateTargetScale(
            this.fitMode,
            targetScale,
            math.v2(node.scale.x, node.scale.y),
            actualDesiredSize
        );
    ...
    private checkTransformReference(
        transformComponent: TransformComponent
    ): Vec2 {
        ...
        const uiTransform = transformComponent.getNodeComponentByType(
            transformReference,
            UITransform
        );
        if (!uiTransform) return null;
        ...
        return math.v2(
            uiTransform.width *
                (this.isConsiderReferenceScale
                    ? transformReference.scale.x
                    : 1),
            uiTransform.height *
                (this.isConsiderReferenceScale ? transformReference.scale.y : 1)
        );
    }
    ...
    private calculateActualSize(referenceSize: Vec2): Vec2 {
        ...
        switch (true) {
            case this.sizeMode === SizeMode.Relative &&
                this.layer === LayerType.UI:
                {
                    desiredSize = this.calculateSizeByRelativeSize(
                        this.desiredSize,
                        referenceSize
                    ).subtract(
                        this.padding.clone().multiplyScalar(this.settings.SCALE)
                    );
                }
                break;

            case this.sizeMode === SizeMode.Relative &&
                this.layer === LayerType.World:
                {
                    desiredSize = this.calculateSizeByRelativeSize(
                        this.desiredSize,
                        referenceSize
            ...
            case this.sizeMode === SizeMode.Constant &&
                this.layer === LayerType.UI:
                {
                    desiredSize = this.desiredSize
                        .clone()
                        .multiplyScalar(this.settings.SCALE);
                }
                break;
    ...
    private calculateSizeByRelativeSize(
        relativeSize: Vec2,
        referenceSize: Vec2
    ): Vec2 {
        return math.v2(
